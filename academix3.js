...
function toggleForms(action) {
  // Map actions to form container IDs
  const formContainerId = (action === 'signup') ? 'sp-form' :
                          (action === 'login') ? 'lg-form' :
                          (action === 'generateToken') ? 'set-form' :
                          (action === 'resetPassword') ? 'newSet-form' : null;

  if (!formContainerId) {
    console.error("Invalid action specified in toggleForms.");
    return;
  }
  ...
  document.querySelectorAll('.form-container').forEach(form => {
    ...
    form.style.display = "none"; // Completely hide the form
  });

  // Show the target form container
  const formToShow = document.getElementById(formContainerId);
  if (formToShow) {
    formToShow.classList.add("active");
    formToShow.style.display = "block"; // Make the form visible
  } else {
    console.error(`Form container with ID "${formContainerId}" not found.`);
    return;
  }

  // Adjust visibility of the login form (`lg-form`) based on the form being toggled
  const lgForm = document.getElementById("lg-form");
  if (lgForm) {
    lgForm.style.display = (formContainerId === "sp-form" || formContainerId === "set-form") ? "none" : "block";
  } else {
    console.error(`Login form container ("lg-form") not found.`);
  }
}
